...
def process_batch(batch_file, emb_column, output_dir, dataset, gpu_id):
    ...
    if dataset == 'ccl':
        ...
        emb_df['cell_line_name'] = df['Model']
        emb_df['drug_name'] = df['Treatment']
...
def main():
    ...
    parser.add_argument('--dataset', type=str, required=True, help='Dataset name ccl or pdx')
    ...
    process_args = [(batch_file, args.embedding_column, args.output_dir, args.dataset, gpu_ids[i % num_gpus]) 
